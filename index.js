...
function setListeners(userCard) {
    ...
    changeBtn.addEventListener('click', () => {
        ...
        const newInput = storage[changeEmail]

        inputName.value = newInput.name
        inputSecondName.value = newInput.secondName
        inputEmail.value = newInput.email
        inputNumber.value = newInput.number
        inputAdress.value = newInput.adress
 ...
 */
function createCard({ name, secondName, email, number, adress }) {
    return `
...
function addCard(e) {
    ...
    const newAdress = document.querySelector('#adress')
    ...
    if (
        ...
        resetInputs(newName, newSecondName, newEmail, newNumber, newAdress)
        return
    }

    const allValuesStorage = Object.values(storage)

    if (allValuesStorage.some((el) => el.email === newEmail.value)) {
        localStorage.removeItem('users')
        delete storage[newEmail]
        localStorage.setItem('users', JSON.stringify(storage))
